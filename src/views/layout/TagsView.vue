<template>
  <div class="tags-view-container" v-show="paneVisible">
    <scroll-pane
      v-show="paneVisible"
      ref="scrollPane"
...
export default {
  ...
  methods: {
    ...
    addTags() {
      ...
      console.log("add", this.$route);
      if (name) {
        this.$store.dispatch("addView", this.$route).then(() => {
          this.moveToCurrentTag();
        });
      }
      console.log("added", this.$store.state.tagsView.visitedViews);
      return false;
    },
    moveToCurrentTag() {
...
span.tags-view-item.router-link-exact-active.router-link-active.active {
